...
// param_type/return_type are always a Tuple (even for 0 or 1 values) and, unlike the Parameter
// instructions themselves, are never affected by DCE removing an unused parameter - so this is
// the only reliable source for a function's signature.
...
// Cranelift signatures return everything through registers, or nothing at all with the caller
// providing a buffer instead (StructReturn) - there's no in-between where only the values that
// don't fit spill out. So any function with more than one return value uses the buffer
// uniformly, even for cases that would technically fit in registers: simpler than replicating
// each target's exact register budget, and correct on all of them.
...
// ReturnValue/GetElementPtr), so this recurses through operands to find one that does -
// mirrors how the type-checker treats these as type-transparent.
//
// A phi can have another phi as one of its incoming edges (e.g. a value threaded through
// nested if/else merges), and which edge happens to be checked first isn't fixed - so this
// tries every incoming edge of a phi, not just the first, tracking which phis are currently
// being resolved (`visiting`) to skip an edge that leads back into one of them rather than
// recursing forever. Only if every edge of a phi leads back to itself with no terminal value
// is that a real problem - see the panic message below for why that happens.
...
fn translate_operation(op: IrOperation, arena: &IrArena, builder: &mut FunctionBuilder, values: &HashMap<InstId, ir::Value>) -> ir::Value {
    ...
    match op {
        ...
        // Cranelift has no float remainder instruction (no target ISA exposes one directly;
        // it'd need a libm fmodf call), so float % is out of scope for now.
        IrOperation::Mod(l, r) =>
            if is_float(l, arena) {
        ...
        // booleans are a plain i8 holding 0/1, so bitwise ops double as logical ones
...
fn translate_instruction(
    ...
    match arena.get_inst(inst_id).clone() {
        // Phi/Parameter are bound to block params before this loop runs; ReturnValue is bound
        // by the FunctionCall right before it; Tuple only ever means something as a Return's
        // return_tuple, handled directly by the terminator translation.
...
fn translate_terminator(
    ...
    match arena.get_term(term_id) {
        ...
        TerminatorInst::Return { return_tuple, .. } => {
            ...
            match sret_ptr {
                // more return values than fit in registers: write each one into the buffer the
                // caller gave us instead of returning them directly.
                Some(ptr) => {
        ...
        // be reached by a well-formed program
