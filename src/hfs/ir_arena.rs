...
impl DominatorTree {
    ...
    */
    ...
    pub fn compute_immediate_dominators(arena: &IrArena, func_id: IrFuncId) -> HashMap<BlockId, BlockId> {
        ...
        let mut rpo_indexes = HashMap::<BlockId, usize>::new();
        ...
        rpo_indexes.insert(start_block, postorder.len());
        ...
        for (rpo_idx, block) in postorder.iter().enumerate() {
            ...
            rpo_indexes.insert(*block, rpo_idx /* because entry was popped */);
        }
            ...
            for curr_block in &postorder {
                ...
                for predecessor in &arena.get_block(*curr_block).predecessors {
                    ...
                    if !immediate_dominators[predecessor].is_null() {
                        ...
                        new_idom = DominatorTree::intersect(&mut immediate_dominators, &rpo_indexes, *predecessor, new_idom);
                    }
                }
    ...
    fn intersect(
        ...
        rpo_indexes: &HashMap<BlockId, usize>,
        block1: BlockId,
        block2: BlockId,
    ) -> BlockId {
        let mut finger1 = (rpo_indexes[&block1], block1);
        let mut finger2 = (rpo_indexes[&block2], block2);
