...
impl IrLowerer {
            ...
            let new_node = match node {
                ...
                TopLevelId::Statement(id) =>
                    return ir_lowerer_error!(
                        ...
                        &self.arena,
                        Some(&self.ast_arena),
                        *self.ast_arena.get_stmt_span(id)
                    ),
            };
    ...
    fn maybe_materialize_array_length(&mut self, type_id: TypeId) -> Result<Option<InstId>, Box<dyn CompileError>> {
        ...
        let length_inst = match self.ast_arena.get_expr(length_expr).clone() {
            Expression::Literal(literal) => {
                let span = *self.ast_arena.get_expr_span(length_expr);
                let entry_block = self.arena.get_func(self.ir_context.curr_func).entry_block;
                self.arena.alloc_inst_for(Instruction::Literal { span, literal }, entry_block)
            },
    ...
    pub fn lower_function_declaration(&mut self, id: FuncId) -> Result<IrFuncId, Box<dyn CompileError>> {
        let (name, param_type, return_type, parameter_exprs, body, span) = {
            ...
            (
                ...
                func_decl.body,
                *self.ast_arena.get_function_span(id),
            )
        };
    ...
    */
    ...
    pub fn lower_if_body(
        ...
        let if_end_block = if new_if_context {
            ...
            self.arena.compare_stacks(
                ...
                merge_spans(stack_after_body.iter().map(|inst| self.arena.get_inst(*inst).get_span()).collect()),
            )?;
    ...
    pub fn generate_merge_phis(&mut self, if_end_block: BlockId, stack_snapshots: &[(BlockId, Vec<InstId>)], span: &Span) {
        ...
        for stack_idx in 0..result_len {
            ...
            if values.iter().all(|v| *v == values[0]) {
                ...
                let mut incoming = IndexMap::<BlockId, InstId>::new();
    ...
    pub fn lower_stmt(&mut self, id: StmtId, curr_block_context: BlockContext) -> Result<(), Box<dyn CompileError>> {
        let span = *self.ast_arena.get_stmt_span(id);
                ...
                if let Some(else_id) = else_stmt {
                    ...
                    match else_stmt {
                        Statement::Else(_) | Statement::ElseIf { .. } => {
                            ...
                            let else_body_block = self.arena.alloc_block(&name, self.ir_context.curr_func);
                            ...
                            self.ir_context.curr_insert_block = else_body_block;
                            ...
                            stack_snapshots.push((self.ir_context.curr_insert_block, else_snapshot));
            ...
            Statement::While { cond, body } => {
                /* jump while_cond_0;
                   ...
                   while_end_0:
                       jump end;
                */
                let while_cond_block = self.arena.alloc_block("while_cond", self.ir_context.curr_func);
                ...
                let cond_inst = self.arena.pop_or_error(
                    merge_spans(cond.iter().map(|stmt| *self.ast_arena.get_stmt_span(*stmt)).collect()),
                    &self.ast_arena,
                )?;
            ...
            */
            ...
            Statement::FunctionCall { arg_count, func_id, is_move, return_values } => {
                ...
                for _ in 0..arg_count {
                    let span = *self.ast_arena.get_stmt_span(id);
                    let arg_inst = self.arena.pop_or_error(span, &self.ast_arena)?;
    ...
    pub fn lower_expr(&mut self, id: ExprId) -> Result<InstId, Box<dyn CompileError>> {
        ...
        let span = *self.ast_arena.get_expr_span(id);
