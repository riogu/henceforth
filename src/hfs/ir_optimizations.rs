...
impl Mem2Reg {
    fn rename_variables(
        ...
        for dom_successor in dom_tree.successors(block_id) {
            // store the state the stack had before we went into a successor block. This has to
            // snapshot every promotable alloca so that we can remove new allocas that appear in a
            // successor's subtrees
