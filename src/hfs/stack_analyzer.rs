...
impl<'a> AstArena<'a> {
    ...
    pub fn popn_or_error(&mut self, n: usize, msg: &str) -> Vec<ExprId> {
        let mut popped: Vec<ExprId> = (0..n)
            .map(|_| self.hfs_stack.pop().unwrap_or_else(|| panic!("{}", msg)))
            .collect();
        popped.reverse();
        popped
    }
    ...
    pub fn validate_return_stack(&mut self, return_type: TypeId) {
        ...
        if expected_count != actual_count {
            panic!(
                "expected {} values on stack for return, found {}",
    ...
    pub fn validate_type(
        ...
        match (actual_type, expected_type) {
            (Type::Tuple(actual_types), Type::Tuple(expected_types)) => {
                ...
                for (i, (&actual_elem_id, &expected_elem_id)) in
...
impl<'a> StackAnalyzer<'a> {
    ...
    pub fn resolve(
        ...
        (
            resolved_top_level,
            stack_parser.arena,
            stack_parser.scope_resolution_stack,
        )
    }
    ...
    fn resolve_top_level(&mut self, nodes: Vec<UnresolvedTopLevelId>) -> Vec<TopLevelId> {
        ...
        for node in nodes.clone() {
            let new_node = match node {
                UnresolvedTopLevelId::VariableDecl(id) => {
                    TopLevelId::VariableDecl(self.resolve_var_decl(id))
                }
                UnresolvedTopLevelId::FunctionDecl(id) => {
                    TopLevelId::FunctionDecl(self.resolve_func_decl(id))
                }
    ...
    fn resolve_var_decl(&mut self, id: UnresolvedVarId) -> VarId {
        ...
        self.scope_resolution_stack
            .push_variable(&unresolved_var.name, var_id);
        var_id
    }
    ...
    fn resolve_func_decl(&mut self, id: UnresolvedFuncId) -> FuncId {
        ...
        self.arena
            .validate_return_stack(self.scope_resolution_stack.get_curr_func_return_type());
    ...
    fn push_function_and_scope_and_alloc(
        &mut self,
        name: &str,
        ...
        self.scope_resolution_stack
            .push_function_and_scope(name, func_id, ret_type);
        func_id
    }
    ...
    fn resolve_stmt(&mut self, id: UnresolvedStmtId) -> StmtId {
        ...
        match unresolved_stmt {
            UnresolvedStatement::If {
                ...
                let cond = self
                    .arena
                    .pop_or_error("expected boolean on stack for if statement");
            ...
            UnresolvedStatement::While { body, cond } => {
                ...
                let cond = self
                    .arena
                    .pop_or_error("expected value on stack for while loop argument");
                ...
                self.arena
                    .alloc_stmt(Statement::While { cond, body }, token)
            }
            UnresolvedStatement::StackBlock(unresolved_expr_ids) => {
                ...
                let stack_change = self
                    .arena
                    .get_stack_change(stack_start, self.arena.hfs_stack.clone());
                for &expr_id in &stack_change {
                    expr_ids.push(expr_id);
                }
                self.arena
                    .alloc_stmt(Statement::StackBlock(expr_ids), token)
            }
            UnresolvedStatement::BlockScope(unresolved_top_level_ids, scope_kind) => {
                ...
                self.arena
                    .alloc_stmt(Statement::BlockScope(top_level_ids, scope_kind), token)
            }
            ...
            UnresolvedStatement::Assignment {
                ...
                let value = if is_move {
                    self.arena
                        .pop_or_error("expected value in stack for move assignment statement.")
                } else {
                    self.arena
                        .last_or_error("expected value in stack for copy assignment statement.")
                };
            ...
            UnresolvedStatement::FunctionCall {
                ...
                for _ in 0..param_types.len() {
                    let arg_expr = self
                        .arena
                        .pop_or_error("expected more elements on stack for function call");
                ...
                self.arena
                    .validate_func_call(func_decl.param_type, arg_type_id);
    ...
    fn resolve_func_call_identifier(&mut self, id: UnresolvedExprId) -> FuncId {
        ...
        match identifier {
            Identifier::GlobalVar(var_id) | Identifier::Variable(var_id) => {
                panic!(
                    "variable '{}' cannot be called as a function.",
    ...
    fn resolve_expr(&mut self, id: UnresolvedExprId) -> ExprId {
        ...
        match self.unresolved_arena.get_unresolved_expr(id).clone() {
            UnresolvedExpression::Operation(unresolved_operation) => {
                self.resolve_operation(&unresolved_operation, token)
            }
            UnresolvedExpression::Identifier(identifier) => {
            ...
            UnresolvedExpression::Literal(literal) => self.arena.alloc_and_push_to_hfs_stack(
            ...
            UnresolvedExpression::Tuple { expressions } => {
                ...
                for expr_id in expressions {
                    ...
                    expr_ids.push(expr_id);
                    self.arena
                        .pop_or_error("[internal error] didnt push to the stack properly");
                    ...
                    if matches!(
                        self.arena.get_expr_provenance(expr_id),
                        ExprProvenance::RuntimeValue
                    ) {
            ...
            UnresolvedExpression::StackKeyword(name) => {
                ...
                let args = match kw_declaration.expected_args_size {
                    Some(n) => self.arena.popn_or_error(
                        n,
                        format!(
                            "expected {} arguments for stack keyword {}",
                            n, kw_declaration.name
                        )
                        .as_str(),
                    ),
    ...
    fn resolve_operation(&mut self, op: &UnresolvedOperation, token: Token<'a>) -> ExprId {
        match op {
            op if op.is_binary() => {
                let (lhs_expr, rhs_expr) = self.arena.pop2_or_error(
                    format!(
                        "expected at least 2 values in stack for binary operation '{:?}'",
                        op
                    )
                    .as_str(),
                );
                let provenance = if matches!(
                    self.arena.get_expr_provenance(lhs_expr),
                    ExprProvenance::CompiletimeValue
                ) && matches!(
                    self.arena.get_expr_provenance(lhs_expr),
                    ExprProvenance::CompiletimeValue
            ...
            UnresolvedOperation::Not => {
                let expr = self.arena.pop_or_error(
                    format!(
                        "expected at least 1 value in stack for unary operation '{:?}'",
                        op
                    )
                    .as_str(),
                );
                ...
                self.arena.alloc_and_push_to_hfs_stack(
                    Expression::Operation(Operation::Not(expr)),
                    provenance,
                    token,
                )
            }
